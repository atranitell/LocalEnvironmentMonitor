...
unsigned char input_type[32] ;
char sendPkt[64] ;

/*********************************************************************
 * FUNCTIONS
 */
void LEMApp_HandleKeys( byte shift, byte keys ) ;
...
void LEMAPP_Init( uint8 task_id )
{
  ...
  LEMAPP_DstAddr.addr.shortAddr = 0;
...
uint16 LEMAPP_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event ) {
        ...
        case ZDO_STATE_CHANGE:
          ...
          if ( (LEMAPP_NwkState == DEV_ZB_COORD)) {
            HalUARTWrite( 0, "&00NetworkFormation$", strlen("&00NetworkFormation$") ) ;
  ...
  /* 定时检查各个传感器的设备信息 */
  if ( events & LEMAPP_SEND_LINK_EVT ) {
    LEMAPP_SEND_REQ_LINKQLT() ;
    osal_start_timerEx( LEMAPP_TaskID, LEMAPP_SEND_LINK_EVT, LEMAPP_SEND_LINK_TIMEOUT ) ;
    return ( events ^ LEMAPP_SEND_LINK_EVT ) ;
  }
  ...
  /* 接收到串口数据 */
   if ( events & LEM_UART_RECV ) {
    ...
    if( strcmp((char*)input_type, "08") == 0 ) {
      LEMAPP_SEND_REQ_CTR( (char*)recvBuf ) ;
    }
    // 控制某端点单片机
    if( strcmp((char*)input_type, "09") == 0 ) {
    }
...
static void LEMAPP_SEND_REQ_CTR( char *addr ) {
  ...
  addr[16] = 0 ;
  HalUARTWrite( 0, (unsigned char*)addr, 16) ;
